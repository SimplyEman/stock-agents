...
_VALID = PeerComparisonReport(
    ...
    peers=["AMD", "AVGO", "TSM"],
    metrics=[],
    subject_advantages=["gross margin"],
    subject_disadvantages=["valuation"],
    rebuttals=["growth justifies the multiple"],
    preference_strength_1_to_10=8,
    reasoning="r",
).model_dump_json()
...
def test_peer_wired_into_pipeline(monkeypatch):
    ...
    peer = PeerComparisonReport(
        ...
        metrics=[],
        subject_advantages=[],
        subject_disadvantages=[],
        rebuttals=[],
        preference_strength_1_to_10=7,
        reasoning="r",
    )
    ...
    def fake_synth(
        ticker, reports, *, peer_report=None, macro=None, forensic_report=None, ctx=None
    ):
    ...
    result = orchestrator.analyze_single_detailed(
        Candidate(
            ticker="NVDA", name="NVIDIA", market_cap_usd=1e12, sector="Tech", industry="Semis"
        )
    )
