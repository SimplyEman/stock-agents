...
def _thesis(**over) -> InvestmentThesis:
    base = dict(
        ticker="NVDA", name="NVIDIA", one_paragraph_summary="s",
        bull_case=["b1", "b2", "b3"], bear_case=["x1", "x2", "x3"],
        what_would_change_my_mind="data center revenue growth decelerating below 20 percent",
        conviction_score=70.0, conviction_label="high",
...
def _snap(thesis, sid="S1", **reports) -> ThesisSnapshot:
    return ThesisSnapshot(id=sid, ticker=thesis.ticker, run_id="R1", taken_at="2026-01-01T00:00:00+00:00",
...
def test_new_red_flag_detected_when_reports_present():
    entry = _snap(
        ...
        fundamentals=FundamentalsReport(
            ticker="NVDA", revenue_cagr_3y=40, revenue_cagr_5y=40, gross_margin_latest=70,
            gross_margin_trend="expanding", operating_margin_latest=50, fcf_conversion=0.9,
            rule_of_40_score=80, segment_analysis="x", peer_comparison="y",
            red_flags=[], yellow_flags=[], score_1_to_10=8, reasoning="r",
        ),
    )
    new = _snap(
        _thesis(conviction_score=70.0), "S2",
        balance_sheet=BalanceSheetReport(
            ticker="NVDA", current_ratio=3.0, shares_outstanding_cagr_3y=1.0, sbc_pct_revenue=5.0,
            fcf_vs_netincome_gap_pct=5.0, goodwill_pct_assets=2.0, debt_maturity_risk="low",
            capital_allocation_grade="A", red_flags=["Customer concentration rising"],
            yellow_flags=[], score_1_to_10=7, reasoning="r",
        ),
    )
    d = D.compute_diff(entry, new)
